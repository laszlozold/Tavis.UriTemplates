...
namespace Tavis.UriTemplates
{
    ...
    public class UriTemplate
    {
        ...
        public Result ResolveResult()
        {
            ...
            var unresolvedVariables = _Parameters
                .Where(p => !result.ParameterNames.Contains(p.Key))
                .ToDictionary(k => k.Key, v => v.Value);

            result.Append(GetQueryString(unresolvedVariables));

            return result;
        }

        private string GetQueryString(IDictionary<string, object> parameters)
        {
            ...
            foreach (var item in parameters)
            {
                ...
                sb.Append(item.Key);
                sb.Append('=');
                sb.Append(HttpUtility.UrlEncode(item.Value.ToString()));
            }
